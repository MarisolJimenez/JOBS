...
RUTA <- "G:/Shared drives/RETAIL REGULAR/PRODUCCION/SAMPLE INSPECTION/45 - HOME/2020019 - JUL20"

INSUMO<-read.csv("C:/Users/jima9001/Desktop/MX/HOME/Consulta_Afi_45.csv") #COMPARTIDO POR SIRVAL
ESTIMADAS <- read.csv("C:/Users/jima9001/Desktop/MX/HOME/ESTIMADAS_45_JUL20.csv") #HECHO A APRTIR DE TMEPLATE


NOM <- "45_PREFACTORES_JUL20"
###########################################################################
NIVEL<- 1
# 0: TIENDA
# 1: CELDA
...
PERIODOS <- c(1018,1023,1027,1032,1036,1040,1045,1049,1053,1058,1062,1066,1071,1075,1079,
              1084,1088,1092,1097,1101,1105,1110,1114,1119,1123,1127)
## 1005,1010,1014,1018,1023,1027,1032,1036,1040,1045,1049,1053,1058,1062,1066,1071,1075,1079,1084,1088,1092,1097,1101,1105,1110,1114
PER_IN <- 1018
##1001,1005,1010,1014,1018,1023,1027,1032,1036,1040,1045,1049,1053,1058,1062,1066,1071,1075,1079,1084,1088,1092,1097,1101,1105,1110
...
## Acv 99116971,53694829,53501171
### total 53501171 ,51535314, 61589667, 51935644 ,54148630, 53412062, 53694829 ,99116971, 51932586 ,72550387, 72550410 ,51935499)
## 52140860
MERCADOS <- "1216147,1216148,1216146,1216163,1216150,1216168,1216155,1216160,1216159,
               ...
               1216154,1216176,1216161,1216174,1216171,1216173,1216172,1216165,1216167"
## ARMS 1342327
##BEVERAGES  1220523,1220851
## PAPELERIAS 1294918
...
##################################################################
INSUMO<-INSUMO%>%select(-UNIDAD,-VALOR,-PC)%>%mutate(NC_PERIODID=NC_PERIODID-989)
INSUMO$AC_NSHOPID<-as.character(INSUMO$AC_NSHOPID)
DWH_FINAL<-DWH_FINAL%>%left_join(INSUMO, by = c("DWH_TPR_ID"="NC_PERIODID","TIENDA"="AC_NSHOPID","F_NAN_KEY"="F_NAN_KEY")) %>%mutate(UNIDAD_SUG=ifelse(is.na(UNIDAD_SUG),VENTAS_UNIDAD,UNIDAD_SUG),VALOR_SUG=ifelse(is.na(VALOR_SUG),VENTAS_VALOR,VALOR_SUG))
...
########################################################################
DWH_RESUMEN <- filter(DWH_FINAL, !is.na(VENTAS_VALOR)) %>% 
  group_by_at(GROUP1)%>%
  summarise(VENTAS_VALOR=sum(VENTAS_VALOR),
            VENTAS_UNIDAD=sum(VENTAS_UNIDAD),
            UNIDAD_SUG=sum(UNIDAD_SUG),
            VALOR_SUG=sum(VALOR_SUG),
            LIN_CORR=sum(LIN_CORR),
            TOT_LIN=sum(TOT_LIN)) %>% filter(VALOR_SUG>0 | VENTAS_VALOR>0)
...
CONSOL <- left_join(TMMS,DWH_RESUMEN, by=c("MSR_TPR_ID"="DWH_TPR_ID", "SHO_EXTERNAL"="TIENDA"))%>%
  ...
  select(SELECT1) %>% filter(VALOR_SUG>=0 | VENTAS_VALOR>=0)%>%
...
fwrite(CONSOL,"PREFACTORES3.csv")
shell.exec("PREFACTORES3.csv")
